...
Number_of_Signals = c(0,70,105,100,205,200)   
Number_of_Nodes= c(0,0,100,100,400,400)  
Data_Transfer_Rate = c(500,700,150,175,980,180) 

df = data.frame(Number_of_Signals,Number_of_Nodes,Data_Transfer_Rate)


data(df)
head(df)
scatterplot3d(x=Number_of_Signals, y=Number_of_Nodes, z=Data_Transfer_Rate)
...
my.lm <- lm(Data_Transfer_Rate~Number_of_Signals+Number_of_Nodes)
...
MLR<- function( X1, X2, Y )
{
	...
	Sum_Y_hat = sum(B0 + B1*Number_of_Signals + B2*Number_of_Nodes)
...
Exp.data = cbind(Number_of_Signals, Number_of_Nodes, Data_Transfer_Rate)

colnames(Exp.data) = c('Number_of_Signals' , 'Number_of_Nodes' , 'Data_Transfer_Rate')

Exp.data

Exp.regression<- MLR( Number_of_Signals, Number_of_Nodes, Data_Transfer_Rate)
